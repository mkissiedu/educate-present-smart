...
interface AssessmentHubProps {
  ...
  onStartAssessment?: (classLevel: string) => void;
  onAssessStudent?: (student: Student) => void;
...
export const AssessmentHub: React.FC<AssessmentHubProps> = ({ 
  ...
  // Use assigned classes or all classes
  ...
  const loadTestPaperCount = async () => {
    try {
      ...
      // Filter by assigned subjects/classes if provided
      const filtered = assignedSubjects && assignedSubjects.length > 0 && assignedClasses && assignedClasses.length > 0
        ? papers.filter(p => 
            assignedSubjects.includes(p.subject as Subject) && 
            assignedClasses.includes(p.grade_level as ClassLevel)
          )
        : papers;
      setTestPaperCount(filtered.length);
    } catch (error) {
  ...
  // Only show classes that are assigned and have students
  const activeClasses = availableClasses.filter(c => classCounts[c] > 0);

  const handleEnterScores = (s: Student) => { setSelectedStudent(s); setScoreModalOpen(true); };
